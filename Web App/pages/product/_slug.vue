<template>
  <div>
    ...
    show-arrows-on-hover cycle>
            <v-carousel-item v-for="(image,index) in productData.images" :key="index"
              :src="`${$axios.defaults.baseURL}/thumbnails/${image}`"
            >
            </v-carousel-item>
          </v-carousel>
        </div>
        <div class="col-md-7 col-sm-7 col-xs-12">
          ...
          <div class="pl-6">
            ...
            <v-card-actions class="pa-0">
              <p class="headline font-weight-light pt-3">&#8377;{{productData.minprice}} <span style="" class="subtitle-1 font-weight-thin">Minimum Price</span></p>
              <v-spacer></v-spacer>
            ...
            <v-btn class="primary white--text" outlined tile dense><v-icon>mdi-cart</v-icon> Buy</v-btn>
            <v-btn class="ml-4" outlined tile>ADD TO COMPARE</v-btn>
...
export default {
  ...
  mounted() {
    this.$axios
      .get(`/product?slug=${this.$route.params.slug}`)
      .then((res) => {
